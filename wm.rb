...
class WindowManager
  ...
  def initialize dpy, num_desktops: 10
    ...
    root.grab_button(true, mask, :async, :async, 0, 0, 1, X11::Form::Mod1)
    root.grab_button(true, mask, :async, :async, 0, 0, 3, X11::Form::Mod1)
...
#       X11::Form::ExposureMask             |
  ...
  def adopt(wid, desktop=nil)
    ...
    begin
      ...
      return w if attr.map_state == 0

      geom = w.get_geometry
      return w if geom.is_a?(X11::Form::Error)
      return w if geom.width < 2 || geom.height < 2 || geom.x < 0 || geom.y < 0
  ...
  def on_map_request(ev)
    ...
    set_focus(ev.window) unless w.special?
  end
  ...
  def on_error(ev) = on_destroy_notify(ev.bad_resource_id)
  ...
  def on_wm_delete_window(wid) = window(wid).destroy
  ...
  def on_net_active_window(wid, ...) = on_map_request(wid)
end
